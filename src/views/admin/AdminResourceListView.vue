...
const productFilteredRows = (rows) => {
  ...
  if (productCategoryFilter.value) {
    nextRows = nextRows.filter((record) => String(record.category_id) === productCategoryFilter.value)
  }
...
const applyQueryState = () => {
  ...
  productCategoryFilter.value = isProductListing.value && typeof route.query.category === 'string' ? route.query.category : ''
  productDateFilter.value = isProductListing.value && typeof route.query.date === 'string' ? route.query.date : ''
  productSort.value = isProductListing.value && typeof route.query.sort === 'string' ? route.query.sort : ''
...
<template>
  <section v-if="resource" class="space-y-6">
    ...
    <div v-if="isProductListing" class="grid gap-4 md:grid-cols-3">
      <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <label class="text-sm font-semibold text-slate-600" for="product-category-filter">Category</label>
        <select
      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <label class="text-sm font-semibold text-slate-600" for="product-date-filter">Date filter</label>
        <select
          id="product-date-filter"
          v-model="productDateFilter"
          class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
        >
          <option v-for="option in productDateOptions" :key="option.value || 'all'" :value="option.value">
            {{ option.label }}
          </option>
        </select>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <select
          ...
          class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
        >
          <option v-for="option in productSortOptions" :key="option.value || 'latest'" :value="option.value">
            {{ option.label }}
          </option>
        </select>
      </div>
    ...
    <div class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      <div class="text-sm text-slate-500">
        Showing {{ paginationRangeStart }}-{{ paginationRangeEnd }} of {{ filteredRows.length }} filtered records
      </div>
      <div class="flex flex-wrap items-center gap-4">
        ...
        <label class="flex items-center gap-3 text-sm font-medium text-slate-700">
          ...
          <select
            v-model="perPage"
            class="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
          >
