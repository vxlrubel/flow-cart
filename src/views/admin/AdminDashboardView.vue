...
<template>
  <section class="space-y-6">
    <div class="bg-linear-to-r from-sky-600 via-cyan-600 to-teal-600 p-6 text-white rounded">
      <p class="text-sm font-semibold uppercase tracking-[0.24em] text-sky-100">Admin Dashboard</p>
      <h1 class="mt-3 text-xl uppercase font-semibold">Flow Cart control center</h1>
      <p class="mt-3 max-w-3xl text-sm text-sky-50">
        Every collection from <code>db.json</code> is available here with create, edit, and
        soft-delete flows.
      </p>
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-3">
      <AdminStatCard
        ...
        detail="Managed resource groups"
      />
      <AdminStatCard
        label="Active Records"
        :value="totalActiveRecords"
        detail="Visible rows across collections"
      />
      <AdminStatCard
        label="Archived Records"
        :value="totalArchivedRecords"
        detail="Soft-deleted rows"
      />
    </div>
      ...
      >
        ...
        <div class="mt-5 flex gap-3">
          <RouterLink
            ...
            class="inline-flex justify-center items-center h-8 bg-sky-500 w-17 text-sm font-semibold text-white transition hover:bg-sky-600 rounded"
          >
            Open
          </RouterLink>
          <RouterLink
            :to="`/admin/${resource.key}/new`"
            class="inline-flex justify-center items-center h-8 border border-sky-500 w-17 text-sm font-semibold text-sky-500 transition hover:bg-sky-50 rounded"
          >
            Create
          </RouterLink>
        </div>
